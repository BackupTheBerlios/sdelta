...
#define  leap(frog) \
  if ( start >= frog ) { \
    from.offset = from.index.natural[where + frog]; \
    if ( memcmp(   to.buffer + to.offset, \
                 from.buffer + from.offset, 0x1000 ) > 0 ) { \
      where  +=  frog; \
      start  -=  frog; \
    } else  start   =  frog - 1; \
  }
...
void  make_sdelta(INPUT_BUF *from_ibuf, INPUT_BUF *to_ibuf)  {
  ...
  FOUND			found;
  unsigned int		count, limit, line, total, where, start;
  u_int16_t		tag;
  QWORD			crc, fcrc;
  SHA_CTX		ctx;
  unsigned char		*here, *there;
...
/*  fprintf(stderr, "to %i\n", to.block);  */
      ...
      from.offset = from.index.natural[where];

/* I coded the below as a while loop,
   however I have never seen it repeating more than once.  */

      while ( ( memcmp(   to.buffer +   to.offset, \
                        from.buffer + from.offset, 0x10 ) > 0 ) &&
              ( from.index.naturals > ++where ) )
        from.offset = from.index.natural[where];
      ...
      while ( memcmp(   to.buffer +   to.offset, \
        ...
        limit             =  MIN ( to.limit, from.limit ) / 8;
...
*/
        found.count++;
        to.offset = match.to_offset + match.total;
        while ( ( to.index.naturals > ++to.block ) &&
                ( to.offset         > to.index.natural[to.block] ) );
      } else  to.block++;
  }
