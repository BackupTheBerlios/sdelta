...
void load_buf(const char *fname, INPUT_BUF *b) {
	...
	if (fname) {
		...
		if (S_ISREG(st.st_mode) || S_ISLNK(st.st_mode)) {
			...
			if ((b->buf = mmap(NULL, b->mmap_size = b->size = alloc_size, PROT_READ, MAP_PRIVATE, b->fd, 0)) == MAP_FAILED)
				...
				fprintf(stderr, "%s: mmap(size = %u, fd = %d) failed\n", fname, alloc_size, b->fd);
			else {
		...
		if ((b->buf = mmap(NULL, alloc_size = b->mmap_size = MAX_MAP_ANON, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0)) == MAP_FAILED)
	...
	/* use MADVISE to cut off the excess */
	if (! b->fd && b->is_mmaped) {
		...
		MADVISE(b->buf, offset, MADV_SEQUENTIAL);
	}
#endif
